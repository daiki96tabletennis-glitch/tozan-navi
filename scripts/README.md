...
- **`azuma`/`mitakesan`の交通情報チェック新規検出**：`check_pages.py`の
  ...
  この2山は`ts-time-val`が無いため、下記の所要時間同期チェック対象からも除外。
- ~~**`trainTimeShinjuku`/`Yokohama`/`Omiya`の孤立フィールド**~~
  → **2026-09-22に修正済み**。当初「山個別ページのHTML/JSからは参照されない
  孤立フィールド」として同期チェックを見送っていたが、実際は**トップページ
  `index.html`の`getTrainTime()`/`getAccessTier()`がソート・電車所要時間
  フィルター・アクセス難易度バッジ(良好/ふつう/要計画)の判定に直接使用**して
  おり、本文の権威データ(`trainAccessHtml`内`ts-time-val`)とのズレが
  156山中84山・121件、うちバッジ表示またはフィルター結果が実際に変わる
  ものが55件見つかった（例: harinokidake 190→280分で「ふつう」→「要計画」
  相当）。`ts-time-val`の値を正とし`mountains.json`の該当3フィールドを
  一括同期。以後`check_pages.py`の「JSON と HTML の情報不一致」チェックに
  この3フィールドの同期チェックを追加し、再発を検知する。
